...
namespace process_monitor.Library
{
    internal static class ProcessMonitorBPFLoader
    {
        ...
        // Note: this must be kept in sync with the C version in proceventebpfext.sys (proceventebpfext.c)
...
#pragma warning restore IDE1006 // Naming Styles
        ...
        private static void Initialize(ILogger logger)
        {
            unsafe
            {
                process_monitor_bpfObject = PInvokes.bpf_object__open("proceventebpfext.sys");
                if (process_monitor_bpfObject == IntPtr.Zero)
                {
                    throw new InvalidOperationException("bpf_object__open for proceventebpfext.sys failed!");
                }
                else
                {
                    logger.LogDebug("SUCCESS: bpf_object__open(proceventebpfext.sys) worked");
                }
                ...
                if (loadResult < 0)
                {
                    throw new InvalidOperationException($"bpf_object__load for proceventebpfext.sys failed with error code: {loadResult}.  Check that the ntosebpfext service is running.");
                }
                else
                {
